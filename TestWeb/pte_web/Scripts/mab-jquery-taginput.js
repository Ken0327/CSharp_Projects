...
; (function ($, window, document, undefined) {
    ...
    Plugin.prototype = {
        init: function (element) {
            ...
            if (usingTypeahead) {
                json = this.options.typeaheadDatasetOptions.member;
            ...
            tagInput.on('keydown typeahead:selected', function (e) {
                ...
                if (e.type === 'typeahead:selected') {
                    if (confirm(' Insert ' + input.val() + ' in tracing list ?') == true) {
                        ...
                        if (allowDuplicates || (tagIndex === -1)) {
                            ...
                            $.each(json.tags.person, function (i, v) {
                                if (v.name == input.val()) {
                                    Insert_Tracing_PTEMember(v.Empid);
                                }
                            });
            ...
            tagInputContainer.on('click', 'span.glyphicon', function (e) {
                ...
                if (confirm(' Remove ' + tagText + ' out tracing list ?') == true) {
                    _removeTagFromDataField(tagData, separator, tagText);
                    tag.remove();
                    
                    $.each(json.tags.person, function (i, v) {
                        if (v.name == tagText) {
                            remove_Tracing_PTEMember(v.Empid);
                        }
                    });
